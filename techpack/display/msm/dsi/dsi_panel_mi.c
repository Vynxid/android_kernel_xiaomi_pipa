...
int dsi_panel_parse_esd_gpio_config(struct dsi_panel *panel)
{
		...
		!strcmp(panel->name,"xiaomi m82 42 02 0b video mode dual dsi dphy panel"))
	{
		...
		if (gpio_is_valid(mi_cfg->esd_err_irq_gpio) && gpio_is_valid(mi_cfg->esd_err_irq_gpio_sec)) {
			...
			pr_debug("dsi_panel_parse_esd_gpio_config esd1:%d, esd2:%d\n", mi_cfg->esd_err_irq_gpio,mi_cfg->esd_err_irq_gpio_sec);
			...
			pr_debug("dsi_panel_parse_esd_gpio_config esd failed!!!!\n");
			rc = -EINVAL;
		}
		return (rc || rc2);
	} else {
				...
				bool enable)
{
	...
	pr_debug("dsi_panel_esd_irq_ctrl_locked 0x%llx\n", panel->mi_cfg.panel_id);
			...
			if (mi_cfg->esd_err_irq_sec) {
				if (enable) {
					if (!mi_cfg->esd_err_sec_enabled) {
						...
						pr_debug("%s panel esd2 irq is enable\n", panel->type);
					}
				} else {
					if (mi_cfg->esd_err_sec_enabled) {
						...
						pr_debug("%s panel esd2 irq is disable\n", panel->type);
					}
				}
			}
		} else {
			pr_debug("%s panel esd2 irq gpio invalid\n", panel->type);
		}
	}
	return 0;
}
...
ssize_t calc_hw_vsync_info(struct dsi_panel *panel, char *buf)
{
	...
	pr_debug("[hw_vsync_info]fps: %d.%d, vsync_period_ns:%lld,"
