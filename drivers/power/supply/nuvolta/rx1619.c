...
void rx1619_get_pmi_icl(struct rx1619_chg *chip)
{
	...
	dev_dbg(chip->dev, "[rx1619] [%s] [rx1619] get icl: %d\n", __func__,
		 val.intval);
...
static void rx1619_fw_download_work(struct work_struct *work)
{
	...
	if (!val.intval) {
		...
		dev_dbg(chip->dev, "[rx1619] %s: FW Version is 0x%x\n",
			 __func__, g_fw_rx_id);
		if (g_fw_rx_id)
			chip->chip_ok = 1;
		else
...
static void rx1619_wireless_int_work(struct work_struct *work)
{
	...
	case 0x05: //sha one ok
		...
		if (g_shaone_data_h == 1) {
			...
			dev_dbg(chip->dev, "[rx1619] [%s] SHA failed ! \n",
				 __func__);
			chip->auth = 0;
		}
		if (!chip->epp)
...
*/
	...
	/*
	   ...
	   dev_dbg(&client->dev, "[rx1619] %s: hw_id is %d\n", __func__, hw_id);
