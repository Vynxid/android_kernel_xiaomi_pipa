...
static int idtp9220_reverse_charge_enable(struct idtp9220_device_info *di)
{
	...
	if (!di->fw_update) {
		...
		if (i < 3) {
			dev_dbg(di->dev, "reverse charging start success\n");
			return 1;
		} else {
			dev_dbg(di->dev, "reverse charging failed start\n");
			idtp9220_set_reverse_enable(di, false);
			return 0;
		}
	} else {
...
retry_chip_fw_show:
	...
	if (di->dt_props.ra9530_enable) {
		if (di->fw_check_flag)
			dev_dbg(di->dev, "chip_fw_show succ.\n");

		if ((!di->fw_check_flag) && (di->retry_num < 2)) {
			di->retry_num++;
			dev_dbg(di->dev, "Retry chip_fw_show, cnt:%d.\n",
				 di->retry_num);
			msleep(500);
		...
		if ((!di->fw_check_flag) && (di->retry_num >= 2)) {
			di->retry_num = 0;
			dev_dbg(di->dev, "chip_fw_show err.\n");
		}
	}
...
static int program_crc_verify_ra9530(struct idtp9220_device_info *di)
{
	...
	if ((data1 != 0x32) || ((data2 != 0xca) && (data2 != 0xba))) {
		dev_dbg(di->dev, "%s: bootloader version err.\n", __func__);
		return false;
	}
		      ...
		      u32 size)
{
	...
	do {
		...
		if ((data & 0x80) != 0x80)
			dev_dbg(di->dev, " check:0x, retry cnt:.\n");
		retry_cnt++;
	...
	do {
		...
		if ((data & 0x1) != 0)
			dev_dbg(di->dev, ":  check:0x, retry cnt:%d.\n",
				 retry_cnt);
		retry_cnt++;
...
*/
	...
	if (di->dt_props.ra9530_enable) {
		pr_debug("RA9530: parse_dt RA9530.\n");
	}
...
static void idtp9220_fw_download_work(struct work_struct *work)
{
	...
	if (di->fw_update) {
		dev_dbg(di->dev, "[idtp9418] [%s] FW Update is on going!\n",
			 __func__);
		return;
	}
	...
	if (di->dt_props.ra9530_enable) {
		if (di->fw_check_flag)
			dev_dbg(di->dev, "fw_download succ.\n");

		if ((!di->fw_check_flag) && (di->retry_num < 2)) {
			di->retry_num++;
			dev_dbg(di->dev, "Retry fw_download_work, cnt:%d.\n",
				 di->retry_num);
		...
		if ((!di->fw_check_flag) && (di->retry_num >= 2)) {
			di->retry_num = 0;
			dev_dbg(di->dev, "fw_download err.\n");
		}
	}
...
static int idt_trx_get_ble_mac(struct idtp9220_device_info *di)
{
	...
	dev_dbg(di->dev, "mac addr of pen: 0x%x:0x%x:0x%x:0x%x:0x%x:0x%x\n",
...
static void idtp9220_irq_work(struct work_struct *work)
{
	...
	if (di->is_reverse_mode || di->is_boost_mode) {
		...
		if (int_val & INT_GET_BLE_ADDR) {
			dev_dbg(di->dev, "[idt] TRX get ble mac\n");
...
static void idt_get_reverse_vout(struct idtp9220_device_info *di)
{
	...
	dev_dbg(di->dev, "[reverse] vout is %d\n", vout);
...
static void idt_get_reverse_soc(struct idtp9220_device_info *di)
{
	...
	if ((soc < 0) || (soc > 0x64)) {
		if (soc == 0xFF) {
			dev_dbg(di->dev, "[reverse] soc is default 0xFF\n");
			di->reverse_pen_soc = 0xFF;
			return;
		} else {
			dev_dbg(di->dev, "[reverse] soc illegal: %d\n", soc);
			return;
		}
	}
	dev_dbg(di->dev, "[reverse] soc is %d\n", soc);
	di->reverse_pen_soc = soc;
	if (di->wireless_psy)
		power_supply_changed(di->wireless_psy);

	if ((di->reverse_pen_soc == 100) && (pen_soc_count < SOC_100_RETRY)) {
		dev_dbg(di->dev, "[reverse] soc is 100 count: %d\n",
			 pen_soc_count);
		pen_soc_count++;
	} else {
		pen_soc_count = 0;
...
static void idt_get_reverse_iin(struct idtp9220_device_info *di)
{
	...
	iin = iin_l | (iin_h << 8);
	dev_dbg(di->dev, "[reverse] iin is %d\n", iin);
	di->reverse_iout = iin;
	return;
}
...
static void idt_get_reverse_temp(struct idtp9220_device_info *di)
{
	...
	dev_dbg(di->dev, "[reverse] temp is %d\n", temp);
	return;
}
...
static void idt_reverse_chg_work(struct work_struct *work)
{
	...
	if (di->is_reverse_mode || di->is_boost_mode) {
		...
		dev_dbg(di->dev, "reverse chg closed, return\n");
		di->reverse_pen_soc = 255;
		di->reverse_vout = 0;
		di->reverse_iout = 0;
		return;
			  ...
			  const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(di->dt_props.hall3_gpio)) {
		...
		if (!hall3_val) {
			dev_dbg(di->dev, "pen online, start reverse charge\n");
			di->hall3_online = 1;
			schedule_delayed_work(&di->hall3_irq_work,
