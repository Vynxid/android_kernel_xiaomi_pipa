...
setup_instance(struct sfax_hw *card)
{
	...
	if (debug & DEBUG_HW)
		pr_debug("%s: got firmware %zu bytes\n",
	...
	if (!err)  {
		...
		sfax_cnt++;
		pr_debug("SpeedFax %d cards installed\n", sfax_cnt);
		return 0;
	}
	disable_hwirq(card);
	free_irq(card->irq, card);
...
sfaxpci_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	pr_debug("mISDN: Speedfax found adapter %s at %s\n",
...
Speedfax_init(void)
{
	...
	pr_debug("Sedlbauer Speedfax+ Driver Rev. %s\n",
		  SPEEDFAX_REV);
