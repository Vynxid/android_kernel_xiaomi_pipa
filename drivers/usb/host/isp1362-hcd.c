...
static int isp1362_mem_config(struct usb_hcd *hcd)
{
	...
	dev_dbg(hcd->self.controller, "ISP1362 Memory usage:\n");
	dev_dbg(hcd->self.controller, "  ISTL:    2 * %4d:     %4d @ $%04x:$%04x\n",
		 istl_size / 2, istl_size, 0, istl_size / 2);
	dev_dbg(hcd->self.controller, "  INTL: %4d * (%3zu+8):  %4d @ $%04x\n",
		 ISP1362_INTL_BUFFERS, intl_blksize - PTD_HEADER_SIZE,
		 intl_size, istl_size);
	dev_dbg(hcd->self.controller, "  ATL : %4d * (%3zu+8):  %4d @ $%04x\n",
		 atl_buffers, atl_blksize - PTD_HEADER_SIZE,
		 atl_size, istl_size + intl_size);
	dev_dbg(hcd->self.controller, "  USED/FREE:   %4d      %4d\n", total,
