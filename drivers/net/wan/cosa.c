...
err_out:
	release_region(cosa->datareg,is_8bit(cosa)?2:4);
	pr_debug("cosa%d: allocating resources failed\n", cosa->num);
	return err;
}
...
static int cosa_net_open(struct net_device *dev)
{
	...
	if (!(chan->cosa->firmware_status & COSA_FW_START)) {
		pr_debug("%s: start the firmware first (status %d)\n",
...
static inline int cosa_download(struct cosa_data *cosa, void __user *arg)
{
	...
	if (!(cosa->firmware_status & COSA_FW_RESET)) {
		pr_debug("%s: reset the card first (status %d)\n",
			  cosa->name, cosa->firmware_status);
		return -EPERM;
	}
	...
	if (i < 0) {
		pr_debug("cosa%d: microcode download failed: %d\n",
			  cosa->num, i);
		return -EIO;
	}
...
static inline int cosa_readmem(struct cosa_data *cosa, void __user *arg)
{
	...
	if (i < 0) {
		pr_debug("cosa%d: reading memory failed: %d\n", cosa->num, i);
		return -EIO;
	}
...
static inline int cosa_start(struct cosa_data *cosa, int address)
{
	...
	if ((cosa->firmware_status & (COSA_FW_RESET|COSA_FW_DOWNLOAD))
		...
		pr_debug("%s: download the microcode and/or reset the card first (status %d)\n",
			  cosa->name, cosa->firmware_status);
		return -EPERM;
	}
	cosa->firmware_status &= ~COSA_FW_RESET;
	if ((i=startmicrocode(cosa, address)) < 0) {
		pr_debug("cosa%d: start microcode at 0x%04x failed: %d\n",
			  cosa->num, address, i);
		return -EIO;
	}
...
static int puthexnumber(struct cosa_data *cosa, int number)
{
	...
	for (i=0; i<4; i++) {
		if (put_wait_data(cosa, temp[i]) == -1) {
			pr_debug("cosa%d: puthexnumber failed to write byte %d\n",
				  cosa->num, i);
			return -1-2*i;
		}
		if (get_wait_data(cosa) != temp[i]) {
			pr_debug("cosa%d: puthexhumber failed to read echo of byte %d\n",
				  cosa->num, i);
			return -2-2*i;
		}
	}
...
static inline void eot_interrupt(struct cosa_data *cosa, int status)
{
...
#ifdef DEBUG_DATA
	{
		...
		pr_debug("cosa%d: unexpected EOT interrupt\n", cosa->num);
	}
	/*
